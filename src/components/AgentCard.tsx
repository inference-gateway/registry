...
export function AgentCard({ agent }: AgentCardProps) {
  ...
  const repository = deriveRepository(agent);
  const docs = deriveDocs(agent);
  const provider = deriveProvider(agent);
  const model = agent.spec?.agent?.model || '';
  const capabilities = agent.spec?.capabilities;
  const toolsCount = agent.spec?.tools?.length ?? 0;
  const skillsCount = agent.spec?.skills?.length ?? 0;

  const copyImageToClipboard = async () => {
    if (!image) return;
    try {
      await navigator.clipboard.writeText(`${image.repository}:${image.tag}`);
      setCopiedImage(true);
  ...
  const copyCommandToClipboard = async () => {
    try {
      await navigator.clipboard.writeText(`infer agents add ${name}`);
  ...
  return (
    <div className="group relative bg-slate-800/40 backdrop-blur-xl rounded-3xl shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 transform hover:-translate-y-2 overflow-hidden border border-slate-700/50 hover:border-blue-500/50 hover:bg-slate-800/60 flex flex-col h-full">
      <div className="relative p-6 flex flex-col flex-1">
        ...
        <div className="relative flex items-start justify-between mb-6 gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-slate-400 text-sm font-medium truncate">
              v{version} • <span className="text-blue-400">{name}</span>
            </p>
          </div>
          {provider && (
            <span className="px-4 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white text-xs font-bold rounded-full shadow-lg shadow-blue-500/25 whitespace-nowrap">
              {provider}
            </span>
          )}
        </div>

        <p className="relative text-slate-300 mb-6 line-clamp-3 leading-relaxed">{description}</p>
        ...
        {capabilities && (
          <div className="relative flex flex-wrap gap-2 mb-6">
            ...
            {capabilities.stateTransitionHistory && (
              <span className="px-3 py-1 bg-amber-500/10 text-amber-300 text-xs font-semibold rounded-full border border-amber-500/30">
                State History
              </span>
            )}
            <span className="px-3 py-1 bg-slate-700/50 text-slate-300 text-xs font-semibold rounded-full border border-slate-600/30">
        ...
        {tags.length > 0 && (
          <div className="relative flex flex-wrap gap-2 mb-6">
            ...
            {tags.length > 8 && (
              <span className="px-2 py-1 text-slate-500 text-xs">+{tags.length - 8} more</span>
            )}
          </div>
        )}
        ...
        <div className="relative border-t border-slate-700/50 pt-6">
          {model && (
            <div className="grid grid-cols-1 gap-4 text-sm mb-4">
              <div>
                <span className="text-slate-400 text-xs uppercase tracking-wider">Model</span>
                <p className="font-mono text-white truncate mt-1">{model}</p>
              </div>
            </div>
          )}

          <div className="mb-4">
            <span className="text-slate-400 text-xs uppercase tracking-wider mb-2 block">
              Add to CLI
            </span>
