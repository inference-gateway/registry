import type { Catalog, CatalogAgent } from '../types/adl';
  ...
  if (!cached) {
    cached = fetch(CATALOG_URL, { headers: { accept: 'application/json' } })
      .then(async (res) => {
        ...
        const invalid = catalog.agents.find((a) => a?.apiVersion !== 'adl.inference-gateway.com/v1');
        if (invalid) {
          throw new Error(
            `catalog contains non-ADL agent (apiVersion=${JSON.stringify(invalid?.apiVersion)})`,
          );
        }
