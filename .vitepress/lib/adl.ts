...
// The CLI resolves any catalog agent by name (URL from spec.server, OCI image from
// spec.deployment), so the short form works for every entry.
export function deriveInstallCommand(agent: CatalogAgent): string {
  return `infer agents add ${agent.metadata.name}`;
}
